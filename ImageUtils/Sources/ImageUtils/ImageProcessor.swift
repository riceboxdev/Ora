...
/// Actor-based image processor for thread-safe image operations
/// Provides optimized compression, resizing, and thumbnail generation
...
public actor ImageProcessor {
    ...
    ///   - maxDimension: Optional maximum dimension to resize before compression
    /// - Returns: Compressed JPEG data, or nil if compression fails
    public func compressImage(_ image: UIImage, maxDimension: CGFloat? = nil) -> Data? {
    ...
    /// - Parameter image: The image to create a thumbnail from
    /// - Parameter maxSize: Maximum size for the thumbnail (default: 400x400)
    /// - Returns: Thumbnail image, or nil if creation fails
    public func createThumbnail(_ image: UIImage, maxSize: CGSize = CGSize(width: 400, height: 400)) -> UIImage? {
        return autoreleasepool {
    ...
    /// - Returns: Tuple containing full image data, thumbnail data, and dimensions, or nil if processing fails
    public func processImage(_ image: UIImage) async -> (fullImageData: Data, thumbnailData: Data, width: Int, height: Int)? {
    ...
    /// - Parameter image: The image to determine compression quality for
    /// - Returns: Compression quality value between 0.0 and 1.0
    ...
    /// - Parameter data: Image data to convert
    /// - Returns: UIImage if data is valid image data, nil otherwise
    public nonisolated func image(from data: Data) -> UIImage? {
